...
ifeq ($(TARGET_USE_HW_CRYPTO),true)
  ENABLE_HW_CRYPTO := ENABLE_HW_CRYPTO=1
else
  ENABLE_HW_CRYPTO := ENABLE_HW_CRYPTO=0
endif
...
ifeq ($(TARGET_USE_RECOVERY),true)
  ENABLE_RECOVERY := ENABLE_RECOVERY=1
else
  ENABLE_RECOVERY := ENABLE_RECOVERY=0
endif
...
$(TARGET_NAND_BOOTLOADER): appsbootldr_clean | $(NAND_BOOTLOADER_OUT)
	$(MAKE) -C $(LK_PATH) TOOLCHAIN_PREFIX=$(CROSS_COMPILE) BOOTLOADER_OUT=$(CROOT_DIR)/$(NAND_BOOTLOADER_OUT) $(BOOTLOADER_PLATFORM) $(SIGNED_KERNEL) $(BOARD_NAME) $(ENABLE_RECOVERY) $(ENABLE_HW_CRYPTO) $(ENABLE_QSEED_SCALAR)

# Top level for eMMC variant targets
$(TARGET_EMMC_BOOTLOADER): emmc_appsbootldr_clean | $(EMMC_BOOTLOADER_OUT) $(INSTALLED_KEYSTOREIMAGE_TARGET)
	$(MAKE) -C $(LK_PATH) TOOLCHAIN_PREFIX=$(CROSS_COMPILE) BOOTLOADER_OUT=$(CROOT_DIR)/$(EMMC_BOOTLOADER_OUT) $(BOOTLOADER_PLATFORM) EMMC_BOOT=1 $(SIGNED_KERNEL) $(VERIFIED_BOOT) $(VERIFIED_BOOT_2) $(VB1_KEY_USED) $(TARGET_BOARD_TYPE_AUTO) $(TARGET_DTBO_NOT_SUPPORTED) $(ENABLE_DISPLAY) $(ENABLE_KASLRSEED) $(ENABLE_BOOTDEVICE_MOUNT) $(DEVICE_STATUS) $(BUILD_VARIANT) $(BOARD_NAME) $(ENABLE_VB_ATTEST) $(OSVERSION_IN_BOOTIMAGE) $(ENABLE_BG_SUPPORT) $(QSEECOM_SECAPP_REGION_2MB) $(TARGET_USE_SYSTEM_AS_ROOT_IMAGE) $(ENABLE_RECOVERY) $(ENABLE_HW_CRYPTO) $(ENABLE_QSEED_SCALAR)
